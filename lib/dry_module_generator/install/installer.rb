...
Dir[File.join(Rails.root, '*', 'lib', '*', 'infra', 'config', 'application.rb')].each do |file|
  ...
  def invalid?(validator, keys)
    if show_error(validator, keys)
      ...
      return if file_content.include?("include Pagy::Frontend")
...
"
    ...
    def update_gemfile
      ...
      unless file_content.include?("sass-rails")
...
gem 'sass-rails'"
      ...
      unless file_content.include?("dry-validation")
        append_to_file(file_path) do
          "
gem 'dry-validation'"
        end
      ...
      unless file_content.include?("dry-struct")
        append_to_file(file_path) do
          "
gem 'dry-struct'"
        end
      ...
      unless file_content.include?("dry-system")
        append_to_file(file_path) do
          "
gem 'dry-system', '~> 1'"
        end
      ...
      unless file_content.include?("dry_struct_generator")
        append_to_file(file_path) do
          "
gem 'dry_struct_generator'"
      ...
      unless file_content.include?("dry_object_mapper")
        append_to_file(file_path) do
          "
...
gem 'pagy'"
      ...
      unless file_content.include?("ransack")
        append_to_file(file_path) do
          "
gem 'ransack'"
        end
